...
fun createLoader(activity: FragmentActivity) : AlertDialog {
    ...
    loader.setIcon(R.mipmap.ic_launcher_read_round)
...
fun uploadImageView(app: ReadApp, imageView: ImageView) {
    ...
    uploadTask.addOnFailureListener { object : OnFailureListener {
        override fun onFailure(error: Exception) {
            Log.v("Read", "uploadTask.exception" + error)
        }
    }
...
fun updateAllReads(app: ReadApp) {
    val userId = app.currentUser!!.uid
    val userEmail = app.currentUser!!.email
    var ReadRef = app.database.ref.child("reads")
        .orderByChild("email")
    val userReadRef = app.database.ref.child("user-reads")
        .child(userId).orderByChild("uid")

    ReadRef.equalTo(userEmail).addListenerForSingleValueEvent(
    ...
    userReadRef.addListenerForSingleValueEvent(
        object : ValueEventListener {
            override fun onCancelled(error: DatabaseError) {}
            override fun onDataChange(snapshot: DataSnapshot) {
                snapshot.children.forEach {
...
fun writeImageRef(app: ReadApp, imageRef: String) {
    val userId = app.currentUser!!.uid
...
fun validatePhoto(app: ReadApp, activity: Activity) {
    ...
    if (googlePhotoExists || imageExists) {
        if(!app.currentUser?.displayName.isNullOrEmpty())
            activity.navView.getHeaderView(0)
                .textView.text = app.currentUser?.displayName
        else
            activity.navView.getHeaderView(0)
                .textView.text = activity.getText(R.string.nav_header_title)
    ...
    {
        activity.navView.getHeaderView(0).imageView.setImageResource(R.mipmap.ic_launcher_read_round)
        uploadImageView(app, activity.navView.getHeaderView(0).imageView)
    }
}

fun checkExistingPhoto(app: ReadApp,activity: Activity) {
